...
_normalized_bgp_community_list = {}
def get_normalized_bgp_community_list_line(line):
    """
    FRR 'bgp community-list' has "seq" field, which defaults to 5 and increments by 5 if unset.
    Unfortunately when using applying configs without the seq field (such as those generated by certain software routers), it will remove the 
    running lines contains "seq n" and re apply the community-list without "seq".
    
    In practice, this brielfy removes the community list completely, which for some router-maps results in a route leak while the reload is in progress.
    Ie, all your transit tagged routes get leaked where it shouldn't go

    Pull request #5169 added seq to community-list, I assume the bug with frr-reload.py has existed since then.
    """
    if ' seq ' in line:
        return line
    res = re.fullmatch('bgp community-list (standard|extended) (\S+) (permit|deny) (\S+)', line)
    if not res:
        return line
    k = res.group(2)
    seq = _normalized_bgp_community_list[k] = _normalized_bgp_community_list.get(k, 0) + 5
    return f'bgp community-list {res.group(1)} {res.group(2)} seq {seq} {res.group(3)} {res.group(4)}'
    ...
    def load_from_file(self, filename):
        ...
        for line in file_output.split("\n"):
            ...
            if line.startswith('bgp community-list'):
                line = get_normalized_bgp_community_list_line(line)
